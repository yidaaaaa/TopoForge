...
## ADR-046 — Web Bambu jobs bind one validated runtime configuration before queueing

- **Date:** 2026-08-03
- **Context:** The official Bambu Studio AppImage and flattened P2S profiles were retained in the repository workspace, but the loopback Web service did not resolve the executable or propagate it to isolated workers. A request could pass the browser form and fail later with `Bambu Studio probe did not resolve an executable path`; supplying only the executable would then defer a second failure because the Web launch did not include the machine, process, and filament profiles required by the official P2S gate.
- **Decision:** Make the loopback server configuration own an optional resolved Bambu executable and an all-or-none machine/process/filament profile set. Validate file existence and executable permission at startup. Before either validation or submission, inject server-owned profiles when the request omits them, probe the selected slicer, return the resolved slicer identity to the API client, and reject unavailable tools with the exact `topoforge web` options needed to correct startup. Propagate the resolved executable to isolated workers through `TOPOFORGE_BAMBU_STUDIO`; keep all slicing and project algorithms in the existing core adapters.
- **Alternatives:** Add the AppImage directory globally to PATH; let workers discover host tools independently; validate only in `/jobs/validate`; accept partial profiles; hard-code repository-relative production paths; defer every configuration error until the slice stage.
- **Impact:** Browser and direct API submissions share one pre-queue gate, retained services expose their effective Bambu configuration through `web --check`, and isolated workers execute the same probed AppImage. A real global workflow now completes official slice, P2S parameter checks, separate project export, archive validation, no-external-profile reopen, and reslice without modifying terrain semantics or bundling host-specific paths into core defaults.

## ADR-047 - Terminal Web job deletion is explicit, bounded, and backup-preserving

- **Date:** 2026-08-03
- **Context:** Durable cancelled and failed jobs remained visible indefinitely because Phase 11 exposed cancellation, backup, cleanup, and restore but no operation that removed a terminal job record or its workspace. Treating cleanup as deletion would be incorrect because cleanup removes only unreferenced content-addressed stages.
- **Decision:** Provide two DELETE modes for terminal jobs: remove the job state/events/logs while retaining the workspace, or remove both the job record and its unshared workspace while retaining adapter-owned backups. Require the full job ID in the request body, reject non-terminal jobs, reject the workspace root and symlink workspaces, require resolved containment below the configured root, and reject workspace deletion while any other job references it. Return measured record/workspace/reclaimed bytes and recheck the filesystem before reporting success.
- **Alternatives:** Hide terminal jobs only in browser state; delete every workspace during cancellation; reuse cleanup as a misleading delete action; permit recursive deletion of arbitrary paths; remove backups with the project; silently delete a workspace still referenced by another job.
- **Impact:** Long-lived local task lists can be maintained without weakening retained evidence. Shared real workspaces remain protected, backups remain available after project deletion, browser controls match manager semantics in Chinese and English, and stale visualization requests are aborted before filesystem removal.
