...
def test_sha256_reads_the_pinned_file_after_final_path_swap(
    ...
    def racing_open(
        ...
        *,
        ...
        names_source = (
            path == source.name if dir_fd is not None else Path(os.fspath(path)) == source
        )
        if not swapped and not swap_blocked and names_source:
            try:
...
def test_json_snapshot_fails_closed_after_parent_path_swap_without_reading_replacement(
    ...
    if os.name == "nt":
        assert swap_blocked is True
        assert swapped is False
        assert source.read_bytes() == b'{"trusted":true}\n'
    else:
        assert swapped is True
...
def test_archive_evidence_uses_pinned_zip_and_primary_handles_after_path_swaps(
    ...
    swapped: set[str] = set()
    swap_blocked: set[str] = set()

    def swap_path(name: str, source: Path, detached: Path, replacement: Path) -> None:
        try:
            source.rename(detached)
        except PermissionError:
            if os.name != "nt":
                raise
            swap_blocked.add(name)
        else:
            replacement.rename(source)
            swapped.add(name)
    ...
    def racing_open_pinned(path: Path, *, label: str) -> Iterator[Any]:
        with real_open_pinned(path, label=label) as pinned:
            if path == project and "project" not in swapped | swap_blocked:
                swap_path("project", project, project_detached, project_replacement)
            elif path == primary and "primary" not in swapped | swap_blocked:
                swap_path("primary", primary, primary_detached, primary_replacement)
            yield pinned
    ...
    if os.name == "nt":
        assert swapped == set()
        assert swap_blocked == {"project", "primary"}
        assert project.read_bytes() != b"not a ZIP"
        assert primary.read_bytes() == primary_payload
    else:
        assert swapped == {"project", "primary"}
        assert swap_blocked == set()
        assert project.read_bytes() == b"not a ZIP"
        assert primary.read_bytes() == b"attacker G-code"
