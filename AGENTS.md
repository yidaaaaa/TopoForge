...
The validated core includes local and no-key global AOI acquisition, printer-aware sampling, orientation, resource preflight, deterministic manufacturing exports, provenance, and slicer evidence. Phase 5 deterministic tiling, seams, connectors, print-local placement, per-tile slicing, and Bambu project roles are complete in 0.4.0. Phase 6 single-user local software is complete in 0.5.0. Phase 7 local checksum-bound GPX/road/river/coast/label/contour overlays is complete in 0.6.0. The current milestone is Phase 8 packaging and release hardening; physical calibration is deferred and non-blocking. API/Web is retained as deferred Phase 9.
