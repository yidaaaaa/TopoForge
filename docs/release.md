...
TopoForge 0.7.0 is the Phase 8 local-use release. It packages the CLI and engine only.
The deferred API/Web plan remains Phase 9 and does not add FastAPI, Uvicorn, a database,
a queue, authentication, or a network listener to this release.

## Verified platform boundary

The retained Phase 8 installation evidence uses CPython 3.12.3 on Linux x86_64 with
...
CLI import outside the repository. The installed CLI creates a synthetic raster, builds
STL/3MF/GLB and reports, and strict-reads the 3MF with zero warnings.

## Connected installation

```bash
python3.12 -m venv ~/.venvs/topoforge-0.7.0
~/.venvs/topoforge-0.7.0/bin/python -m pip install \
  /PATH/TO/topoforge-0.7.0-py3-none-any.whl
...
mkdir -p topoforge-0.7.0-wheelhouse
/tmp/topoforge-wheelhouse-tools/bin/python -m pip download \
  --dest topoforge-0.7.0-wheelhouse \
  /PATH/TO/topoforge-0.7.0-py3-none-any.whl
sha256sum topoforge-0.7.0-wheelhouse/* > topoforge-0.7.0-wheelhouse/SHA256SUMS
sha256sum -c topoforge-0.7.0-wheelhouse/SHA256SUMS
```

On the offline workstation:

```bash
cd /PATH/TO/topoforge-0.7.0-wheelhouse
sha256sum -c SHA256SUMS
python3.12 -m venv ~/.venvs/topoforge-0.7.0
~/.venvs/topoforge-0.7.0/bin/python -m pip install \
  --no-index --find-links . topoforge==0.7.0
~/.venvs/topoforge-0.7.0/bin/topoforge doctor
```
...
3. Install 0.7.0 into a new environment rather than mutating the old one.
...
Installed CLI rollback keeps the 0.7.0 environment intact and switches back to 0.6.0:

```bash
~/.venvs/topoforge-0.6.0/bin/topoforge doctor
ln -sfn ~/.venvs/topoforge-0.6.0/bin/topoforge ~/.local/bin/topoforge
```

For a source checkout after the Phase 8 release commit and tag:

```bash
git revert --no-edit v0.7.0
```
